<template>
    ...
    -->
      ...
      -->
      <div v-if="year > 1861 && temp != 0" class="anomaly">
        {{ temp }}
      </div>
      <div class="credits">
       Baseline period for temperature averages: 1951-1980 <br>
       Based on projections of CNRM-CERFACS CNRM-CM6-1-HR model output <br>
       prepared for CMIP6 ScenarioMIP ssp245 (v20191202) and historical (v20191021)
      </div>
  </div>
</template>
...
.credits { text-align: left; position: absolute; left: 15px; bottom: 12px; font-size: 0.75em; opacity:0.7;}
</style>
...
export default {
  ...
  mounted () {
    ...
    this.playerInterval = setInterval( () => {
      if (this.$refs.video) {
        ...
        this.year = 1861 + current
        if (this.global_means.length) {
          // this.glowMaterial.uniforms.glowColor =
          let v = Math.min(1.0, THREE.MathUtils.mapLinear(this.global_means[current].climatology, -1.5, 1.5, 0.0, 1.0))
  ...
  methods: {
    ...
    loadGlobalMeansData () {
      var self = this;
      var file = 'data/global-means.csv';
    ...
    render () {
      this.renderer.clear();
      if (this.glow) {
        //let viewVector = new THREE.Vector3().subVectors( this.camera.position, this.glow.getWorldPosition());
      //  this.glow.material.uniforms.viewVector.value = viewVector;
      }

      /*

      if (this.year) {
        let v = (this.year % 10)/10 || 0
        let r = v > 0.5 ? Math.floor( (v-0.5) * 255 * 2) : 0
        let g = v > 0.5 ? Math.abs(Math.floor((0.5 - v) * 200)) : Math.abs(Math.floor((0.5 - v) * 300))
        let b = (1 - v) > 0.5 ? Math.floor(((0.5 - v)) * 255 * 2) : 0
        let color = new THREE.Color('rgb(' + r + ',  ' + g + ', ' + b + ')');
      ...
      */
      ...
      // this.glowMaterial.uniforms.glowColor.value = new THREE.Color('#ffffff').sub(new THREE.Color(d3.interpolateRdBu((this.year % 10)/10)))

      // console.log( d3.interpolateRdBu((this.year % 10)/10))
      this.renderer.render(this.scene, this.camera);
